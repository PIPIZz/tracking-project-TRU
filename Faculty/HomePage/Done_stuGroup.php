                ...
                $result = $stmt->fetchAll();
                 $i = 1;
                      foreach($result as $val){
                        $Term = $val['Term'];
                        $Year =$val['Year'];
                        $sql1 = "SELECT * FROM project Where Pro_BA =1 AND Year = '$Year' AND Term = '$Term'AND Step = 6 ";
                        $stmt1 = $conn->prepare($sql1);
                        $stmt1->execute();
                        $count1 = $stmt1->rowCount();

                        $sql2 = "SELECT * FROM project Where Pro_BA =2 AND Year = '$Year' AND Term = '$Term' AND Step = 6";
                        $stmt2 = $conn->prepare($sql2);
                        $stmt2->execute();
                        $count2 = $stmt2->rowCount();

                        $sql3= "SELECT * FROM project Where Pro_BA =3 AND Year = '$Year' AND Term = '$Term' AND Step = 6 ";
                        $stmt3 = $conn->prepare($sql3);
                        $stmt3->execute();
                        $count3 = $stmt3->rowCount();

                        $sql4 = "SELECT * FROM project Where Pro_BA =4 AND Year = '$Year' AND Term = '$Term' AND Step = 6 ";
