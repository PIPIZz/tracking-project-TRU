                    ...
                    <div style="border-radius: 0px;" class="form-horizontal group-border-dashed">
                        ...
                        <p class="text-right">
                            <button data-toggle="modal" data-id="<?php echo $res['T_id']; ?>"
                                class="btn btn-primary btn-lg ModalEdit_Proflie_btn">
                                <h4>แก้ไข</h4>
                            </button>
...
<?php 
    include('../Modal/M_EditProfile_Teacher.php'); 
    include('../Modal/M_Edit_Password_Admin.php');
?>
...
$(document).ready(function() {
    $('.ModalEdit_Proflie_btn').click(function() {
        ...
        // alert(StuID);
        $.get('../Process_teacher/P_Get_T.php?T_ID=' + T_ID,
            function(data) {
                // alert(data);
                var result = JSON.parse(data);
                console.log(result);
                // alert(result);
                $('#mod-EditProfileTeacher').modal('show');
                for (var i in result) {
